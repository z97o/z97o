...
def _to_date(value):
    """Convert Excel date or string to Python date safely."""
    if pd.isna(value):
        return None
...
class Command(BaseCommand):
    help = "Import Omantel NetInsight dataset from Excel into the database."

    def add_arguments(self, parser):
        parser.add_argument("--excel", required=True, help="Path to Excel file (NetInsight_Large_Detailed_Dataset.xlsx)")
    ...
    def handle(self, *args, **options):
        path = options["excel"]
        regions_sheet = options["regions_sheet"]
        towers_sheet = options["towers_sheet"]
        complaints_sheet = options["complaints_sheet"]
        geo_sheet = options["geo_sheet"]
        ...
        # ---------------------- REGIONS ----------------------
        self.stdout.write(f"Importing sheet: {regions_sheet}")
        ...
        # ---------------------- TOWERS ----------------------
        self.stdout.write(f"Importing sheet: {towers_sheet}")
        ...
        # ---------------------- COMPLAINTS ----------------------
        self.stdout.write(f"Importing sheet: {complaints_sheet}")
        ...
        # ---------------------- GEO & CLIMATE ----------------------
        self.stdout.write(f"Importing sheet: {geo_sheet}")
